// [Missing Code 1] เพิ่มไฟล์ header ที่จำเป็น
#include "lab20.h"
#include <iostream>
...
int main(){
    srand(time(0)); // ตั้งค่า seed สำหรับสุ่มค่า
    ...
    // [Missing Code 2] สร้าง Equipment 4 ประเภท ได้แก่ sword, axes, shield, armor
    ...
    while(true){
        ...
        if(player_action == 'C'){
            ...
            cin >> eq;
             ...
             // [Missing Code 3] สวมใส่ Equipment ตามที่ผู้ใช้เลือกใหม่
            if(eq == '1') hero.equip(&sword);
            else if(eq == '2') hero.equip(&axes);
            else if(eq == '3') hero.equip(&shield);
            else if(eq == '4') hero.equip(&armor);
